...
const Create = ({ isOpen, close, initialValues }) => {
  ...
  const { loading, setLoading, setProducts, products } = useContext(ItemsContext);

  const text = fields[location.state];

  const submit = (values) => {
    const createReq = createItem(values, products, setLoading, setProducts, close, 'products');
    return createReq;
  };
  ...
  return (
    <FormModal isOpen={isOpen} style={customStyles} close={close}>
      ...
      <Form initialValues={initialValues} submit={submit} buttonText={text} loading={loading}>
